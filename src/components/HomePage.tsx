...
export function HomePage({ ctx, setRoute }: HomePageProps) {
  ...
  const nextDeadline = { label: 'Voter roll updates close', date: 'May 3, 2026', days: Math.max(1, Math.ceil((new Date('2026-05-03').getTime() - today.getTime()) / 86400000)) };
  ...
  return (
    <div className="page" style={{ animation: 'fade-up 0.5s' }}>
      ...
      <p className="subhead" style={{ marginTop: 10 }}>
        General elections to the {ctx.state || 'Karnataka'} Legislative Assembly · polling on <b style={{ color: 'var(--ink)' }}>14 May 2026</b>.
      ...
      <div className="grid" style={{ gridTemplateColumns: 'repeat(12, 1fr)', marginTop: 28 }}>
        ...
        <div className="card ink" style={{ gridColumn: 'span 7', padding: 26, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', opacity: .6 }}>Polling day</div>
          <div style={{ fontFamily: "'Geist', sans-serif", fontWeight: 500, fontSize: 84, lineHeight: 1, marginTop: 8, letterSpacing: '-0.04em' }}>
            {daysOut}<span style={{ fontSize: 24, opacity: .6, marginLeft: 8 }}>days</span>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10, fontSize: 11.5, opacity: .6 }}>
        ...
        <div className="card" style={{ gridColumn: 'span 5' }}>
          ...
          <button className="btn sm" style={{ marginTop: 12 }} onClick={() => setRoute('chat')}>Ask the assistant about Form 8 →</button>
        </div>
          ...
          <div style={{ display: 'grid', gap: 8, gridTemplateColumns: 'repeat(2, 1fr)' }}>
            {[
              ...
              <button key={q} className="btn" style={{ justifyContent: 'flex-start', fontSize: 13, textAlign: 'left' }}
                onClick={() => { window.__seedChat = q; setRoute('chat'); }}>
      ...
      <div style={{ marginTop: 40 }}>
        ...
        <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', marginTop: 10 }}>
          {[
            ...
            { t: 'Disputed results & hung assemblies', s: "What happens if no one wins outright?", r: 'chat', accent: 'var(--lilac)' },
            { t: 'NRI postal voting (ETPBS)', s: "Voting from abroad — step by step.", r: 'chat', accent: 'var(--accent)' },
          ].map(c => (
            <button key={c.t} className="card flat" style={{ textAlign: 'left', border: '1px solid var(--line)', cursor: 'pointer' }}
              onClick={() => {
                if(c.r === 'chat') { window.__seedChat = c.t; }
                setRoute(c.r as RouteId);
              }}>
