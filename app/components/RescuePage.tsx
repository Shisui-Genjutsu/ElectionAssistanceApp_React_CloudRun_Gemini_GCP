    ...
    <div className="page" style={{ animation: "fade-up 0.5s" }}>
      ...
      <div className="card" style={{ marginTop: 22, padding: 28, borderLeft: "4px solid var(--warn)" }}>
        ...
        <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
          {s.canStillDo.map((o, i) => (
            <div key={i} style={{
              ...
              <button className="btn sm" onClick={() => setRoute?.("chat", { prompt: `Can you explain more about this option: "${o.t}"? Specifically, how do I "${o.action}"?` })}>{o.action} →</button>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 20, display: "flex", gap: 10 }}>
          <button className="btn primary" onClick={() => setRoute?.("chat", { prompt: `I am facing an issue: "${s.q}" Can you explain my fallback options in detail?` })}>Discuss with Saksham →</button>
