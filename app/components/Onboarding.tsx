            ...
            <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))" }}>
              {PERSONAS.map((p) => (
                <button key={p.id}
                  ...
                  <div style={{ fontSize: 13, opacity: 0.72, lineHeight: 1.4 }}>{p.blurb}</div>
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 28, alignItems: "center" }}>
              <button className="btn primary" disabled={!persona} onClick={() => setStep(1)}
                style={{ opacity: persona ? 1 : 0.4 }}>
                Continue →
              </button>
              <span className="muted" style={{ fontSize: 12.5 }}>No login. Nothing stored on servers. You can change this anytime.</span>
            </div>
          </div>
        )}
